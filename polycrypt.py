...
def crack_password(target_hash, wordlist_path, max_password_length):
    ...
    def detect_hash_type(target_hash):
        try:
            ...
            if (
                target_hash.startswith("$2a$")
                or target_hash.startswith("$2b$")
                or target_hash.startswith("$2y$")
            ):
                return "bcrypt"
        ...
        with open(wordlist_path, "r") as wordlist_file:
            for word in wordlist_file:
                ...
                if match:
                    ...
                    if len(password) <= max_password_length and passlib_bcrypt.verify(
    ...
    if hash_type == "bcrypt":
        with ThreadPoolExecutor() as executor:
            results = list(
                executor.map(
                    lambda algo_func: crack_with_bcrypt(*algo_func),
                    [(wordlist_path, target_hash, max_password_length)],
                )
            )
        ...
        with open(wordlist_path, "r") as wordlist_file:
            for password in wordlist_file:
                ...
                for transformed_password in transformed_passwords:
                    hashed_transformed_password = hash_password(
                        transformed_password, hash_function
                    )
    ...
    with ThreadPoolExecutor() as executor:
        results = list(
            executor.map(
                lambda algo_func: crack_with_algorithm(*algo_func),
                [(hash_type, hash_algorithms[hash_type], max_password_length)],
            )
        )
...
wordlist_path_label = ttk.Label(
    wordlist_path_frame, text="Wordlist Path:", style="Label.TLabel"
)
...
browse_button = ttk.Button(
    wordlist_path_frame, text="Browse", command=browse_wordlist_path
)
browse_button.pack(side="right")
browse_button.place(x=420, y=11)

browse_button_style = ttk.Style()
browse_button_style.configure(
    "Browse.TButton", foreground="white", background="#b8b8b8"
)
browse_button_style.map("Browse.TButton", background=[("active", "#45a049")])
browse_button["style"] = "Browse.TButton"

max_password_length_label = ttk.Label(
    frame, text="Max Password Length:", style="Label.TLabel"
)
...
crack_button_style.configure(
    "CrackButton.TButton", foreground="white", background="#b8b8b8"
)
